...
% figure(3); imshow(Y/max(Y(:))); title(["Reconstructed Image (In noiseless case) " + ...
...
% figure(4); imagesc(dctCoeffsCompressed2); title(["Quantized DCT coefficient matrix"+ ...
%     "(The block size is " + blockSize2 + " \times " + blockSize2 + ")", ...
%     "(The local threshold is used to achieve the compression ratio of " + compressionRatio1 + ")"]); colormap(gray);
% figure(5); imshow(Y2/max(Y2(:))); title(["Reconstructed Image (In noiseless case) " + ...
